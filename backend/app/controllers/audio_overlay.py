...
def main(video_filepath, time_dicts):
    ...
    temp_filename = (
        base64.urlsafe_b64encode(video_filepath.encode("utf-8")).decode("utf-8")
        + ".wav"
    )
    ...
    original_video.write_videofile(
        video_filepath.replace("uploaded_files", "output_video")
    )
...
def overlay(original_audio, time_dict):
    ...
    while audio_idx < end:
        ...
        original_audio = original_audio.overlay(
            link_audio, position=audio_idx * 1000, times=3
        )
